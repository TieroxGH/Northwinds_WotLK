...
public:
    ...
    struct boss_four_horsemenAI : public BossAI
    {
        ...
        {
            if (me->GetHealth() * 100 / me->GetMaxHealth() == 66 || me->GetHealth() * 100 / me->GetMaxHealth() == 33)
            {
                if (canBlock == 0)
                {
                    canBlock++;
                ...
                if (canBlock == 1)
                {
                    me->CastSpell(me, SPELL_SHIELD_WALL, false);
                    canBlock = 2;
                }
            }
            if (me->GetHealth() * 100 / me->GetMaxHealth() == 65 || me->GetHealth() * 100 / me->GetMaxHealth() == 32)
            {
                canBlock = 0;
            }
        }
